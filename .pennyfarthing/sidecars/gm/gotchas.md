...
- **ALWAYS run the orchestrator render/R2 scripts via `uv run python scripts/...` — never bare `python3`.** `generate_poi_images.py`, `generate_portrait_images.py`, `generate_music.py`, `r2_sync_packs.py`, `r2_manifest_from_bucket.py` all import `boto3` (R2 scan/upload), which lives in the orchestrator's `uv` env. System `python3` (Homebrew 3.14) has no boto3 and is PEP-668 externally-managed, so `pip install` is blocked too. Symptom: `ModuleNotFoundError: No module named 'boto3'` from `render_common.existing_r2_keys()`. Gotcha within the gotcha: `--force` skips the R2 scan, so a `--force` run can succeed under bare `python3` and lull you into thinking it's fine — the next non-force run then fails. The `sq-poi` / `sq-music` skill docs still show bare `python3` (stale); the auto-mode classifier blocks the GM from editing those slash-command files, so flag it for Keith rather than retrying.
- **POI/portrait images live per-WORLD on R2, not per-genre.** `render_common` routes `image_subdir="poi"|"portraits"` to `worlds/<world>/assets/<subdir>/<slug>.png`. The `sq-poi` skill's "default output `genre_packs/{genre}/images/poi/`" line is misleading — don't pass `--output-dir`, let the world-scoped routing fire. Rendered PNGs are git-ignored; `r2_sync_packs.py --files <abs paths>` uploads, then `r2_manifest_from_bucket.py` rebuilds the committed `sidequest-content/r2_manifest.json` from a live bucket scan.
- **`cover_poi` in `world.yaml` IS the hero shot, and it must match a real POI slug.** Server `rest.py:list_genres` builds `hero_image` as `.../assets/poi/<cover_poi>.png` and serves from `cdn.slabgorb.com` by default — a slug typo (e.g. slugifying the display name "The Gate…" → `the_emerald_city_gate` when the real slug is `emerald_city_gate`) yields a silent 404 and the lobby falls back to the placeholder glyph. When a hero shot "isn't working but the image is on the server," diff `cover_poi` against the `history.yaml` slug and HEAD the R2 key before assuming a missing render.
