"""Caption images using Claude's vision via claude CLI subprocess.

Generates LoRA training captions for art images. Claude understands art
history, composition, and style — produces better captions for fine art
than Florence-2 ever could.

Usage:
    python3 scripts/caption_claude.py /path/to/images/
    python3 scripts/caption_claude.py /path/to/images/ --trigger ukiyoe_style
    python3 scripts/caption_claude.py /path/to/images/ --metadata /path/to/metadata/
    python3 scripts/caption_claude.py /path/to/images/ --overwrite
    python3 scripts/caption_claude.py /path/to/images/ --dry-run
"""

from __future__ import annotations

import argparse
import base64
import json
import logging
import subprocess
import sys
import time
from pathlib import Path

log = logging.getLogger(__name__)

CAPTION_PROMPT = """Describe this image for training a LoRA style model. Focus on:
- Visual composition (foreground, background, framing)
- Color palette and technique (ink, watercolor, woodblock, etc.)
- Subject matter (what is depicted — people, landscapes, objects)
- Artistic style elements (line weight, flat color, perspective, patterns)
- Mood and atmosphere

Write a single paragraph, 2-4 sentences. Be concrete and visual — describe what you SEE, not what you interpret. Do not mention the artist's name or the title. Do not start with "This image shows" or "The image depicts."
"""


def caption_image(image_path: Path, trigger_word: str = "") -> str:
    """Caption a single image using claude CLI."""
    result = subprocess.run(
        [
            "claude", "-p", CAPTION_PROMPT,
            "--output-format", "text",
            str(image_path),
        ],
        capture_output=True,
        text=True,
        timeout=60,
    )

    if result.returncode != 0:
        raise RuntimeError(f"claude CLI failed: {result.stderr.strip()}")

    caption = result.stdout.strip()
    if trigger_word:
        caption = f"{caption} {trigger_word}"

    return caption


def enrich_with_metadata(image_dir: Path, metadata_dir: Path) -> int:
    """Merge structured metadata into captions."""
    enriched = 0
    for txt_path in sorted(image_dir.glob("*.txt")):
        meta_path = metadata_dir / f"{txt_path.stem}.json"
        if not meta_path.exists():
            continue

        caption = txt_path.read_text().strip()
        meta = json.loads(meta_path.read_text())

        parts = []
        if meta.get("artists"):
            parts.append(f"by {meta['artists'][0]}")
        if meta.get("dated"):
            parts.append(meta["dated"])
        if meta.get("title"):
            parts.append(f'"{meta["title"]}"')

        if parts:
            prefix = ", ".join(parts)
            txt_path.write_text(f"{prefix}. {caption}")
            enriched += 1

    return enriched


def main():
    parser = argparse.ArgumentParser(description="Caption images with Claude vision")
    parser.add_argument("directory", help="Directory of images to caption")
    parser.add_argument("--trigger", help="Trigger word to append")
    parser.add_argument("--metadata", help="JSON metadata dir for enrichment")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing captions")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--limit", type=int, help="Max images to caption (for testing)")
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s: %(message)s")

    image_dir = Path(args.directory).resolve()
    if not image_dir.is_dir():
        log.error("Not a directory: %s", image_dir)
        sys.exit(1)

    image_files = sorted(
        p for p in image_dir.iterdir()
        if p.suffix.lower() in {".jpg", ".jpeg", ".png", ".webp"}
    )
    log.info("Found %d images in %s", len(image_files), image_dir)

    existing = sum(1 for p in image_files if p.with_suffix(".txt").exists())
    to_caption = len(image_files) if args.overwrite else len(image_files) - existing

    if args.limit:
        image_files = image_files[:args.limit]

    if args.dry_run:
        print(f"\nDry run:")
        print(f"  Images: {len(image_files)}")
        print(f"  Already captioned: {existing}")
        print(f"  Would caption: {to_caption}")
        print(f"  Trigger: {args.trigger or '(none)'}")
        return

    completed = 0
    skipped = 0
    failed = 0
    start_time = time.monotonic()

    for i, img_path in enumerate(image_files, 1):
        txt_path = img_path.with_suffix(".txt")

        if txt_path.exists() and not args.overwrite:
            skipped += 1
            continue

        try:
            img_start = time.monotonic()
            caption = caption_image(img_path, args.trigger or "")
            txt_path.write_text(caption)
            elapsed = time.monotonic() - img_start
            completed += 1
            log.info("[%d/%d] %s (%.1fs)", i, len(image_files), img_path.name, elapsed)
        except Exception as e:
            failed += 1
            log.error("[%d/%d] %s FAILED: %s", i, len(image_files), img_path.name, e)

    total_time = time.monotonic() - start_time

    print(f"\nCaptioning complete:")
    print(f"  Completed: {completed}")
    print(f"  Skipped: {skipped}")
    print(f"  Failed: {failed}")
    print(f"  Elapsed: {total_time:.1f}s")
    if completed > 0:
        print(f"  Avg: {total_time / completed:.1f}s per image")

    if args.metadata:
        metadata_dir = Path(args.metadata).resolve()
        if metadata_dir.is_dir():
            enriched = enrich_with_metadata(image_dir, metadata_dir)
            print(f"  Enriched {enriched} captions with metadata")
        else:
            log.error("Metadata dir not found: %s", metadata_dir)


if __name__ == "__main__":
    main()
