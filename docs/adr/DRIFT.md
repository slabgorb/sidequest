...
| [ADR-111: Recency-Zone Narrator Guardrails Migrate to Tool Descriptions and Primacy-Cached Output Prose](111-narrator-guardrails-into-tool-descriptions.md) | ✓ accepted | sprint/current-sprint.yaml#57-4 — **confrontation slice resolved independently by 61-18**: the `confrontation_trigger` guardrail was dead prose on the SDK path (suppressed in `build_narrator_prompt`, never migrated when its `begin_confrontation` target was retired in 59-4). Story 61-18 extracted `CONFRONTATION_TRIGGER_CORE` (framing-neutral single source) and composed it into the IntentRouter `_SYSTEM_PROMPT` (ADR-113 owns confrontation triggering on the SDK path), keeping the legacy narrator constant byte-identical. The broader ADR-111 migration (the other three guardrails landed in 57-4) stays deferred only insofar as the snapshot-slimming/recency-zone retirement work it shares lineage with (ADR-110/112) is incomplete. |
