...
1. **Inspecting a save must never mutate it (the WAL/save-clobber hazard).**
   ...
   (`forensic_query._ro_connect` docstring, `forensic_query.py`). Opening a
...
2. **A corrupt or partial save must never 500 the page.** Forensics exists
   ...
   (`forensic_query.py`, `forensic_fold.py`).
...
(`forensic_query.build_turn_bundle`, `forensic_query.py`; PG port
`pg/forensic.py`). The bundle keys are:
...
`_events_for_round` → `build_timeline`, `forensic_query.py`). An unknown
round, or a round with no events, returns the **empty bundle** rather than
raising (`forensic_query.py`).
...
1. **`fold_known_facts`** (`forensic_fold.py`) — folds the trusted event
   ...
   (`forensic_fold.py`); the footnote/KnownFacts stream (ADR-100) is the only
   ...
   still fold.
2. **`fold_turn_telemetry`** (`forensic_fold.py`) — curates a round's
   ...
   no seq to record).
3. **`fold_mechanical_census`** (`forensic_fold.py`) — folds this round's
   ...
   `forensic_fold.py`). Per PC: no prior → `baseline`; prior == current →
   `static`; else `moved` with typed deltas computed by `_pc_deltas`
   (`forensic_fold.py`) over location, edge/HP, xp, level, inventory
   (name-aggregated set-diff), and acquired advancements. Session trope state is
   diffed separately (`forensic_fold.py`). The round's rolled-up `state`
   is `absent` / `static` / `moved`.

A fourth fold, `fold_mechanical_strip` (`forensic_fold.py`), computes a
...
(`forensic_query.py`) are retained as the forward (Phase-3) server-side
...
(`mechanical_census.py`) writes, per **seated PC every round** (sealed
...
(`pg/forensic.py`) is the Postgres reader: lock-free MVCC pooled reads
...
plain dicts and are engine-agnostic (`pg/forensic.py`).
...
1. **Strict `?mode=ro` — no WAL clobber.** `_ro_connect` opens
   ...
   (`forensic_query.py`). Opening a WAL-mode save read-only materializes
   ...
   raising (`forensic_query.py`). Under Postgres the equivalent invariant
   is "reads never lock and never write": plain MVCC pooled connections only
   (`pg/forensic.py, 310-318`).
2. **Loud-skip, never raise.** A corrupt save must never 500 the forensics page.
   ...
   `[]` (`forensic_query.py`), `_safe_json` → `{"__unparseable__": raw}`
   sentinel (`forensic_query.py`) — and every fold records bad rows in
   `unparseable_seqs` rather than aborting (`forensic_fold.py, 144-194,
   228-258`). The PG reader adds `_safe_json_logged` so the snapshot/encounter
   reads log the sentinel too (`pg/forensic.py`). Census *emission* is
   ...
   drops the others or the trope row (`mechanical_census.py`).
3. **Truth-tier separation / amber-badge.** Three tiers stay distinct and must
   ...
   it as distinct from the stored snapshot (`forensic_query.py`;
   `forensic_fold.py`). This is ADR-014's honesty doctrine applied to
...
4. **Census lockstep with rounds.** Exactly one census per seated PC per round,
   ...
   the turn transaction (`mechanical_census.py`). Read-side, the per-round
   ...
   round < ?` (`forensic_query.py`; `pg/forensic.py`).
5. **Lexical-sort dependence on timestamp normalization.** Round bucketing is a
   ...
   *different* formats — `events` is Python `.isoformat()` (`T` separator,
   ...
   correct (`forensic_query.py`). Under Postgres **both** columns are
   ...
   is deliberately *not* applied (`pg/forensic.py, 134-172`).
...
`component='mechanical'` (`mechanical_census.py, 188` → `publish_event`,
`telemetry/watcher_hub.py`), so it lands in the same `turn_telemetry`
...
stream is visible at the save-select level (`forensic_query.py`;
`pg/forensic.py`). The reader emits its own loud-log warnings on every
...
- **Event-source mechanical state instead of a census (backfill / derive from
  ...
  `SCRAPBOOK_ENTRY` and carry no `state_delta` (`forensic_fold.py`), so per-PC
...
- **ADR-031 (Game Watcher — Semantic Telemetry):** defines the
  ...
  **new `component='mechanical'` sub-stream** within that substrate
  (`mechanical_census.py, 188`); ADR-031 establishes *that* telemetry exists
...
- **ADR-100 (Journal Pipeline Coherence — Footnotes/KnownFacts):** owns the
  ...
  output (`forensic_fold.py, 54-101`).
