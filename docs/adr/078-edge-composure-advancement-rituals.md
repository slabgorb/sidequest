...
**Deferred to ADR-081 (Advancement Effect Variant Expansion v1) and ADR-082+ (deferred variants):** additional `AdvancementEffect` variants requested by GM but requiring substantial new plumbing. ADR-081 covers the two load-bearing at Tier 1 (`AllyEdgeIntercept`, `ConditionalEffectGating`). The remaining variants — `AllyBeatDiscount` (party-aware `resolved_beat_for`), `BetweenConfrontationsAction` (new game-state slot), `AllyEdgeGrant` (scene-scope ally lookup), `EdgeThresholdDelay` (conditional threshold firing), and the broader T2/T3 set — remain deferred to ADR-082+. Heavy_metal Lore/Craft tiers ship with `LoreRevealBonus` at tier 1 and explicit `mechanical_effects: []  # TODO ADR-082+` placeholders at tiers 2-3. The empty arrays are documented stubs with a follow-up ticket, not silent half-shipping. ADR-082+ is filed after story 8's playtest acceptance gate clears.
...
2. **Extended `AdvancementEffect` variants.** GM proposed five new variants for non-combat affinities (Craft, Lore). **Ruling: `LoreRevealBonus` ships in v1** — single field, no plumbing change. **`AllyBeatDiscount`, `BetweenConfrontationsAction`, `AllyEdgeGrant`, `EdgeThresholdDelay` deferred to ADR-082+** — each requires substantial new context plumbing. (ADR-081 landed two Tier 1 variants — `AllyEdgeIntercept`, `ConditionalEffectGating` — separately, driven by Evropi starting kits.) Heavy_metal Lore/Craft tiers 2-3 ship with documented `mechanical_effects: []  # TODO ADR-082+` stubs.
