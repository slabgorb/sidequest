...
Wiring is end-to-end, not just present: `sidequest/server/websocket_session_handler.py` calls `should_recompute_arc(snapshot.turn_manager.interaction)` after the interaction bump, then `:1013` calls `recompute_arc_history(...)`, then seeds promoted-chapter lore (`:1014-1029`).

### OTEL — the lie detector for the arc

Per CLAUDE.md's OTEL Observability Principle, the recompute emits spans defined in `sidequest/telemetry/spans/world.py`:

- `world_history.arc_tick` (`world_materialization.py`) — fires on **every** cadence call, carrying `from_maturity`/`to_maturity`, `chapters_before`/`chapters_after`, `tier_changed`, and `cadence_interval`. A stable-tier no-op is still observable, so the GM panel can confirm the tick ran rather than silently skipped.
