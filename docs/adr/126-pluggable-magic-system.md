...
`sidequest/magic/models.py:75` `MagicWorking`). It just never got its own ADR.
...
`sidequest/magic/plugin.py:21` defines `MagicPlugin` as a `@runtime_checkable`
`typing.Protocol`, not an ABC. A plugin class is anything that structurally
satisfies it:

- `plugin_id: str` — the registry key (`plugin.py:25`).
- `required_attrs() -> set[str]` — the plugin-specific `MagicWorking` fields that
  MUST be populated for this kind of magic (`plugin.py:27`). All three shipped
  plugins return `set(descriptor.required_span_attrs)`, sourcing the answer from
  the plugin's YAML descriptor (e.g. `innate_v1.py:46`).
- `validate_working(working, config) -> list[Flag]` — plugin-side validation
  producing yellow/red/deep_red flags; an empty list means clean (`plugin.py:30`).
...
`MAGIC_PLUGINS: dict[str, MagicPlugin]` (`plugin.py:37`) is a bare module-level
...
(`innate_v1.py:102`, `item_legacy_v1.py:84`, `learned_v1.py:103`). The package
...
submodules at import — the module docstring at `plugin.py:8` calls this out
...
for spans (`plugin.py:9-11`).

A `get_plugin(plugin_id)` lookup helper (`plugin.py:40`) raises a `KeyError` that
...
- **`innate_v1`** (`innate_v1.py`) — character-as-source magic (source `innate`).
  ...
  `flavor → consent_state` coherence table (`innate_v1.py:35`, e.g. `acquired` and
...
- **`learned_v1`** (`learned_v1.py`) — prepared-spell magic for caster classes
  ...
  attrs: `spell_id` and `slot_level` (`slot_level < 1` is RED, `learned_v1.py:68`).
  Lane respect rejects item mechanisms (`discovery`, `mccoy`, `relational`,
  `faction`) and innate mechanisms (`native`, `condition`) as RED
  (`learned_v1.py:34-40`, `:77-98`); learned magic must use `studied` or
  `granted`.
- **`item_legacy_v1`** (`item_legacy_v1.py`) — items as agents
  ...
  `item_legacy_v1.py:56`). Lane respect: a `native`-mechanism working is a RED
  violation because native delivery is innate territory — items must be
  carried/found/built (`item_legacy_v1.py:71`).
...
(`validator.py:25-31`) so a world config referencing an unbuilt plugin is rejected
with a clean DEEP_RED rather than crashing — see Invariants.

### The flag-severity model

`FlagSeverity` (`models.py:270`) is a three-level `StrEnum`: `yellow`, `red`,
`deep_red`. A `Flag` (`models.py:276`) carries `severity`, a machine-readable
`reason`, and a human `detail`. The top-level `validate(working, config)`
(`validator.py:34`) composes two layers and returns the accumulated `list[Flag]`
(empty = clean):

1. **Framework-side checks** (`validator.py:41-103`):
   ...
   - hard-limit keyword match against `narrator_basis` (a deliberately crude v1
     substring detector, `validator.py:87-103`) → DEEP_RED
     `hard_limit_violation:<id>`.
2. **Plugin-side validation** (`validator.py:105-124`): resolve the plugin via
...
`validator.py:126-134`, even DEEP_RED flags surface in OTEL but do **not** halt
...
- **Import-time registration is the only registration.** A plugin is registered
  ...
  same contract as `SPAN_ROUTES` (`plugin.py:8-11`).
- **`required_attrs` / `validate_working` are the whole plugin contract.** A
  ...
  `@runtime_checkable` Protocol (`plugin.py:21-31`). `required_attrs()` declares
...
- **`get_plugin` fails loud.** Lookup of an unregistered id raises a `KeyError`
  enumerating the registered set (`plugin.py:40-47`); the validator catches it
  specifically to convert a forward-looking-but-unbuilt plugin into a clean
  DEEP_RED rather than an unhandled crash (`validator.py:109-123`).
- **WorldKnowledge awareness ordering: `local_register` ≤ `primary`.**
  `WorldKnowledge` (`models.py:31`) carries a `primary` awareness tag and an
  ...
  (`_AWARENESS_ORDER`, `models.py:21-28`). A `model_validator` enforces that
  `local_register` cannot exceed `primary` on that axis (`models.py:58-69`): a
  ...
  (`models.py:39` and throughout) per No Silent Fallbacks — an unknown field is a
...
- The validator carries a `_PLUGIN_SOURCE` map (`validator.py:25-31`) that mirrors
  data living on the YAML `Plugin` descriptor (`models.py:259` `source`) because
...
- Hard-limit detection is a crude substring match in v1 (`validator.py:87-103`),
  with acknowledged false-positive risk for short keywords; smarter detection is
  deferred.
- DEEP_RED does not yet interrupt narration (`validator.py:126-134`), so a world
...
- **Folding magic into the `RulesetModule` seam (ADR-117).** Rejected: the two
  ...
  list, `models.py:295`), unlike the one-module-per-session ruleset binding. Merging
...
- **Explicit `register()` call / decorator instead of import side effect.**
  ...
  (`plugin.py:8-11`) and keep the registry completeness test shape identical to
  the telemetry one.
...
- **ADR-097 (Class Mechanical Surface) — names the slot, does not govern the
  ...
  plugin fills the slot" (`097-class-mechanical-surface.md:49`, and the framing at
