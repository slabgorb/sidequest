...
- **`DaemonUnavailableError`** — raised when the media daemon is unreachable. Caught at the dispatch boundary; sessions continue. See `sidequest-server/sidequest/server/websocket_session_handler.py` (rendering), `sidequest-server/sidequest/game/lore_embedding.py` (lore embedding).
- **`skipped_daemon_unavailable: bool`** — explicit "we tried, daemon was down, we skipped" signal that propagates through dispatch results so OTEL can record it. See `lore_embedding.py` and the dispatch wrapper in `dispatch/lore_embed.py`.
- **`render.skipped reason=daemon_unavailable`** OTEL events — the GM panel can see degradation rather than guessing whether a subsystem ran. See `websocket_session_handler.py` and 3113.

**Philosophy shift since the original 2026-03-25 design.** The original ADR specified three-step fallback chains (primary → fallback → last-resort) for every subsystem. The codebase has converged on a simpler two-pattern model, partly because the original chains were masking real problems — see the cautionary comment at `websocket_session_handler.py`: *"silent fallback is why grimvault renders looked generic."* This is consistent with CLAUDE.md's `<critical>No Silent Fallbacks</critical>` principle.
...
- **Heuristic-as-primary** for tasks with a code-only path. `AudioInterpreter` (`sidequest-server/sidequest/audio/interpreter.py`, instantiated module-level at `session_handler.py`) is the live primary for audio cue extraction. The originally-specified `MusicDirector` agent is dormant — only `MusicDirectorDecision` (the Pydantic protocol model in `audio/protocol.py`) survives, and `rest.py` reports `"has_music_director": False`. If a MusicDirector agent is ever restored it would slot in as a primary above the interpreter; currently the interpreter *is* the primary.
