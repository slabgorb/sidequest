...
7. **Async look-ahead materializer + frontier-crossing promotion** — wiring test + OTEL. ✓ **SHIPPED — ADR-106 CLOSED (verified, not asserted).** Plan 7 (server PR #306 → develop `bcabbf6`) shipped the five-stage materializer + wiring-tested worker but left a sanctioned honest deferral: `register_lookahead_worker` had **zero live-session callers** (`observers=0` on live `frontier.region_transition` spans — the dungeon did not grow in a real game). The session-integration follow-up (resolution spec/plan `2026-05-17-beneath-sunden-plan-7-session-integration*`, §14) closes it: `session_integration.attach/detach_dungeon_from_session` wired into the real WS lifecycle (`handlers/connect.py` open, `server/websocket_session_handler.py` cleanup teardown), connect-time Seed=Expansion-0 bootstrap, write-once `campaign_seed`, curate→Anthropic-SDK one-shot, and a save-keyed cross-session double-register guard. A **verified pre-existing Plan-4 content gap** (4 genre-level set-piece trope_ids referenced by the Plan-4 themes but defined nowhere → a real `beneath_sunden` connect aborted) was fixed by authoring those 4 tropes in genre-level `caverns_and_claudes/tropes.yaml` (content branch `feat/beneath-sunden-trope-gap-fix`). **Closure evidence (independent final adversarial review, self-generated):** the keystone `tests/dungeon/test_session_lifecycle_wiring.py` drives the **real** `GenreLoader(DEFAULT_GENRE_PACK_SEARCH_PATHS).load("caverns_and_claudes")` pack (zero `_attach_pack` fabrication) — bootstrap commits expansion 0+1, a **real** `GameSnapshot.apply_world_patch` region crossing materializes the next expansion (`max(expansion_id)` grows ≥2), and the live `frontier.region_transition` span carries **`observers >= 1`** (the ADR-106 lie-detector flipped off zero); full server suite 6482 passed / 0 failed; `lookahead_worker.py` / `frontier_hook.py` byte-unchanged (blob-equality proven). Branches gated to finish-branch (server `feat/beneath-sunden-plan-7-session-integration` + content `feat/beneath-sunden-trope-gap-fix` — must merge together). As-built carry-forward: the §14.D guard claims the save key **after** a successful bootstrap+register (retry-safe; the at-entry-check / claim-after-success await-window is a reviewed, in-code-documented, spec-aligned tradeoff bounded by the single-`SessionRoom`-per-save production architecture). See resolution spec/plan §14.
