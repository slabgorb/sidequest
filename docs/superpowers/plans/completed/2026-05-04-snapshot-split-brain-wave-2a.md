...
> **COMPLETED via story 45-52 (commit `ec968ec`, 2026-05-12) — checkbox state never updated.** All Wave 2A deliverables shipped: `NpcPoolMember` lives at `sidequest-server/sidequest/game/npc_pool.py`; `Npc` gained `pool_origin`, `last_seen_location`, `last_seen_turn`; `GameSnapshot.npc_registry` dropped and replaced with `npc_pool`; 3-step lookup with `npcs_hit/pool_hit/invented` strategies in `narration_apply.py:1219+`; `SPAN_NPC_REFERENCED` registered in `telemetry/spans/npc.py:73` with `SPAN_ROUTES` entry; migration `_migrate_s2_npc_registry_split` in `migrations.py:73`; chassis fold removed. Remaining `NpcRegistryEntry` / `npc_registry` text in tree is migration code + historical comments. Plan body left intact as historical reference. Proactive pool seeding is a deliberate follow-up, not part of Wave 2A.
