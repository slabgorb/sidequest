...
**D4 read-only contract test (final, empirically validated).** The falsifiable RO guard for `list_saves` is the **WAL-flip / sidecar-creation** discriminator, not a byte/perm snapshot (both proven vacuous on a clean checkpointed DB; `SqliteStore.open` is byte-idempotent on the *main* file and macOS permits it on a `0444` db). `SqliteStore.open` *always* runs `PRAGMA journal_mode=WAL`, materializing `save.db-wal`/`-shm` beside even a cleanly-closed save; `_ro_connect` (`?mode=ro`) on a cleanly-checkpointed save creates no sidecar. Test: seed → `.close()` clean → `assert before == {"save.db"}` (fixture-clean sanity; fails loudly if a future close() leaves a stray `-wal` rather than silently weakening) → `list_saves` → `assert {slugs} == {"ro_test"}` and `assert after == {"save.db"}`. RED on `SqliteStore.open` (sidecars appear), GREEN on `_ro_connect`. Both halves empirically verified during execution.
